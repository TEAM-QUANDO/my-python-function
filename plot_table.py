...
df = pd.read_csv('I:/Programming/Jupyter/callinsight_May.csv', encoding='cp932')
image = plot_table(df)
'''
...
def plot_table(data: pd.DataFrame,
    ...
    # 行列数
    r = len(data)
    row_height = plt_get_text_height_pixel(
        fontsize) * (1 if r > 2 else 2) + 4 + 4
    ...
    if title is not None and title != '':
        title, color = (title, None) if type(
            title) is str else (title[0], title[1])
        plt.title(title, ax=ax, color=color, fontsize=fontsize + 2)
        top = plt_get_text_height_pixel(fontsize + 2) + 4 + 4

    table = ax.table(cellText=data[1:], colWidths=col_widths, bbox=[
                     0, 0, 1, 1], colLabels=data[0], **kwargs)
